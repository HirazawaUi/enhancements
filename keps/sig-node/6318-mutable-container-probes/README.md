...
As an operator investigating a crash loop, I want to remove or relax a liveness probe on the affected Pod so that I can inspect the live container without replacing it and losing the state needed for diagnosis.

#### Story 2 (Optional)

As a workload owner, I want to tune readiness and startup probe periods, timeouts, and thresholds after observing the application under real load so that I can correct the health-check behavior without causing an otherwise unnecessary rollout.
